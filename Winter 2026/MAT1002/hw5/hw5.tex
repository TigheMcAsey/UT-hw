...
\begin{document}
    ...
    \begin{pb}
        ...
        \end{align}
        Now we finally compute that \(G\) satisfies \eqref{desiderata}, to do so we compute the weak derivative \(\Delta G\), since \(\phi_{n,m}\) are dense in \(C^\infty(T^2)\) we know that \(\Delta G\) is uniquely determined by its ``Fourier coefficients'', we can compute these as follows, (once again enumerating \(\mathbb{Z}^{2} \setminus \set{(0,0)}\)) we take \(S_N\) to be the \(N\)-th partial sum of Fourier coefficients, then \(S_N \to G\) in \(L^2\), so that by definition of weak derivative, \(\Delta G(1) = \gen{G,\Delta 1} = \gen{G,0} = 0\).
    \end{pb}
    \begin{pb}
        \textcolor{red}{TODO}
    \end{pb}
