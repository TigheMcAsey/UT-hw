...
\begin{document}
    ...
    \begin{pb}
        ...
        To prove this, it suffices to show that \(\pi: (X,A) \to (X/A,A/A)\) induces the isomorphism on homology, since then we get \(\pi_* [q] = \pi_*\), and thus \([\hat{f}] = \pi_*[q][f] = \pi_*[f] = [f']\). To see that \(\pi: (X,A) \to (X/A,A/A)\) induces the isomorphism on homology, we use the following diagram, for \(U\) the open set containing \(A\) that retracts to \(A\)
        \begin{equation*}
            \begin{tikzcd}
