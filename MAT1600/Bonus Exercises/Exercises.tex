...
\begin{document}
    ...
    \begin{pb}\textbf{(Durrett 3.7.6)}
        ...
        \end{align*}
        For sufficiently large \(n\), we have \(x/n < 1\), so that we can remove the minimums, since the above is differentiable almost everywhere for every \(n\), each distribution gives rise to a probability density so our application of Scheffe's theorem is fine. This lets us write the distribution as
        \begin{align*}
