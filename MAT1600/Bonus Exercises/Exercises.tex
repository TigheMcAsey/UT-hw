...
\begin{document}
    ...
    \begin{pb}\textbf{(Durrett 2.5.11)}
        Let \(\epsilon > 0\), then suppose that for some sequence \(n_p\), we have \[\min_{k \leq n_p}P\left(\abs{\sum_k^{n_p}x_j} < n \epsilon\right) \to 0\] \(k_p := \text{argmin}_{k\leq n_p}P\left(\abs{\sum_k^{n_p}x_j} < n \epsilon\right)\) Then since we have
        \begin{align*}
            \frac{\abs{S_{n_p}-\sum_{k_p}^{n_p}X_j}}{n_p} = \frac{\abs{S_{k_p}}}{n_p}
        \end{align*}
        we can conclude that
        \begin{align*}
            P(\abs{S_{k_p}} > n_p \epsilon) \leq 
        \end{align*}
    \end{pb}
