...
\begin{document}
    ...
    \end{pb}
    \begin{pb}
        \textbf{(a)} Since real inner products are symmetric it suffices to show one case, so assume \(v_1\wedge\cdots\wedge v_p = \alpha \neq 0\). Now, it will suffice to show by induction that if \(k < p\), then we can choose \(\omega_{k+1}\), so that
        \begin{align*}
            \left\{\begin{pmatrix}
